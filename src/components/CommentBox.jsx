...
export default function CommentBox({ handleLike, handleDelete, comment, score, liked, idx }) {
    ...
    return (
        <Card key={idx} style={{ margin: 10, backgroundColor: color, borderRadius: 5, padding: 2, display: 'flex', flexDirection: 'column' }}>
            <div style={{ color: 'white' }}>{comment}</div>
            {score && <div style={{ color: 'white' }}>Google Sentiment Score: {score}</div>}
